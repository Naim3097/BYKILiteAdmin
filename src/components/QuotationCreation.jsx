    ...
    <div className="space-y-6">
          ...
          <div className="md:hidden p-3 space-y-3">
            {getFilteredQuotations().length > 0 ? (
              getFilteredQuotations().map(quote => (
                <div key={quote.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                  <div className="flex items-start justify-between gap-3 mb-3 pb-3 border-b border-gray-100">
                    <div className="min-w-0 flex-1">
                      <div className="font-mono text-sm font-semibold text-gray-900">{quote.quotationNumber}</div>
                      <div className="text-sm font-medium text-gray-900 mt-1 break-words">{quote.customerName}</div>
                      <div className="text-xs text-gray-500">{quote.vehicleInfo?.make} {quote.vehicleInfo?.model}</div>
                      <div className="text-xs text-gray-400 mt-1">{formatDate(quote.dateCreated)}</div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-lg font-bold text-gray-900">{formatCurrency(quote.total)}</div>
                      <span className={`inline-block mt-1 px-2 py-0.5 text-[10px] font-semibold rounded-lg border ${getStatusColor(quote.status)}`}>{quote.status?.toUpperCase()}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <button onClick={() => openViewModal(quote)} className="py-2 text-xs font-semibold bg-blue-50 text-blue-700 rounded min-h-touch">View</button>
                    <button onClick={() => handleEditButtonClick(quote)} className="py-2 text-xs font-semibold bg-orange-50 text-orange-700 rounded min-h-touch">Edit</button>
                    <button onClick={() => downloadPDF(quote)} className="py-2 text-xs font-semibold bg-red-50 text-red-700 rounded min-h-touch">PDF</button>
                  </div>
                </div>
              ))
            ) : (
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 ...
                 <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm flex flex-col justify-center">
                    ...
                    <div className="mt-8 flex flex-col-reverse sm:flex-row gap-3">
                      <button onClick={() => setViewMode('list')} className="flex-1 py-3 text-gray-600 font-medium hover:bg-gray-50 rounded-lg transition-colors border border-gray-200 min-h-touch">
                        Cancel
                      </button>
                      <button 
                        onClick={handleSaveQuotation} 
                        disabled={isSaving}
                        className="flex-1 py-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all disabled:opacity-50 min-h-touch"
                      >
      ...
      <ResponsiveModal isOpen={showCustomerModal} onClose={() => setShowCustomerModal(false)} title="Select Customer" size="md">
         <input 
           ...
           className="w-full px-4 py-3 border border-gray-200 rounded-lg mb-4 focus:ring-2 focus:ring-blue-100 text-base"
           value={customerSearchTerm}
           onChange={(e) => setCustomerSearchTerm(e.target.value)}
         />
         <div className="max-h-[60vh] sm:max-h-72 overflow-y-auto space-y-2">
           {filteredCustomers.map(cust => (
             <div 
               key={cust.id} 
               onClick={() => selectCustomer(cust)}
               className="p-3 border border-gray-100 rounded-lg hover:bg-blue-50 hover:border-blue-200 cursor-pointer transition-all min-h-touch"
             >
      ...
      {showViewQuotationModal && selectedQuotationForView && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
            <div className="p-4 sm:p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sticky top-0 bg-white z-10">
               <div>
                  <h3 className="text-lg sm:text-xl font-bold text-gray-900">Quotation #{selectedQuotationForView.quotationNumber}</h3>
                  <p className="text-sm text-gray-500">{formatDate(selectedQuotationForView.dateCreated)}</p>
               </div>
               <button onClick={() => setShowViewQuotationModal(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg self-end sm:self-auto min-h-touch">Close</button>
            </div>
